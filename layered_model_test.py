...
def test_define_dense_model_single_layer():
    ...
    assert model.input_shape == (None, 43), " input_shape is not correct"
    assert model.output_shape == (None, 1), " output_shape is not correct"


def test_define_dense_model_with_hidden_layer():
    model = define_dense_model_with_hidden_layer(43, activation_func_array=['relu', 'sigmoid'], hidden_layer_size=11,
                                                 output_length=1)
